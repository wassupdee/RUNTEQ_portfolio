...
- 誕生日・大切な日のリマインド通知は、LINEのメッセージで届く<br>（この為、設定をONにする必要があるプッシュ通知と違い、快適に活用できる）
- 上記に加えて、プロフィールやアルバム機能を備えている

#### サブ
- 通知のタイミングを、誕生日・大切な日の最長60日前に設定できる<br>（この為、イベントやプレゼントの準備の時間を確保しやすい。競合は最長1週間前の通知）
- プロフィール情報に「最後に連絡した日」を登録できる<br>（どの程度連絡を取っていない期間があるかを可視化し、継続的な連絡を取るきっかけを提供する）
